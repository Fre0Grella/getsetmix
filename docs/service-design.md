...
- Lightweight target: < 1 GB RAM for the server

## User flow

1. Open the service from a domain or local app.
2. Click the add button that Paste the URL from your clipboard (single track or playlist/set).
3. Server fetches metadata and validates the source; UI shows the detected source and metadata.
...
5. Repeate for every amount of songs you want
6. Press a button to Start a batch download. Show per-track status (queued, downloading, tagging, ingested, error) plus overall batch progress.
7. When complete,open rekordbox and reload the Rekordbox XML source to see new tracks in the Inbox playlist.
